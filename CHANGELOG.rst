...
- Support for ``commonName`` in certificates has been dropped.
  It has been deprecated for a while now and isn't  supported by any major browser.
- The oldest supported pyOpenSSL version (if the ``pyopenssl`` backend is used) is now 17.0.0.
  When using such an old pyOpenSSL version, you have to pin *cryptography* yourself.
